...
from torchvision.transforms import Resize, ToTensor, Normalize, Compose, CenterCrop, ColorJitter, Grayscale, RandomCrop
...
class MikeDataset(MaskBaseDataset):
    def __init__(self, data_dir, transform=None, mode='train', val_ratio=0.2):
        self.data_dir = data_dir
        self.val_ratio = val_ratio
        self.mode =mode
        self.image_paths = []
    ...
    def setup(self):
        ...
        if self.mode == 'train':
            for profile in profiles:
                ...
                id, gender, race, age = profile.split("_")
                if self.data_label_distribution[str(gender + age)] !=0:
                    self.data_label_distribution[str(gender + age)] -= 1
                    continue
                img_folder = os.path.join(self.data_dir, profile)
                for file_name in os.listdir(img_folder):
...
class MaskSplitByProfileDataset(MaskBaseDataset):
    ...
    def setup(self):
        self.profiles = os.listdir(self.data_dir)
        self.profiles = [profile for profile in self.profiles if not profile.startswith(".")]
        self.pre_labels = []
        self.image_paths = []
        ...
        for profile in self.profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                self.gender_labels.append(gender_label)
                self.age_labels.append(age_label)
            self.pre_labels.append(self.gender_labels[-1] * 3 + self.age_labels[-1])
...
class TestDataset(Dataset):
    def __init__(self, img_paths, resize, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246)):
        ...
        self.transform = VMycrop_transformer
